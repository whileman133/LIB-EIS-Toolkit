...
class FunctionControlAlg(pybamm.models.submodels.external_circuit.base_external_circuit.BaseModel):
    """
    ...
    def __init__(self, param, external_circuit_function, options, **kwargs):
        super().__init__(param, options)
        self.external_circuit_function = external_circuit_function

    def get_fundamental_variables(self):
        # Current is a variable
        i_var = pybamm.Variable("iapp variable [A]", scale=self.param.Q)
        I = i_var

        variables = {
            "iapp variable [A]": i_var,
            "iapp [A]": I,
            "iapp [C-rate]": I / self.param.Q,
        }
    ...
    def set_initial_conditions(self, variables):
        ...
        i_cell = variables["iapp variable [A]"]
...
class EISSimulation:
    ...
    def __init__(
        ...
        V_scale = getattr(self.model.variables["vcell [V]"], "scale", 1)
        I_scale = getattr(self.model.variables["iapp [A]"], "scale", 1)
    ...
    def set_up_model_for_eis(self, model):
        ...
        V_cell = pybamm.Variable("vcell variable [V]")
        new_model.variables["vcell variable [V]"] = V_cell
        V = new_model.variables["vcell [V]"]
        # Add an algebraic equation for the voltage variable
        new_model.algebraic[V_cell] = V_cell - V
        new_model.initial_conditions[V_cell] = new_model.param.pos_Uocp0 - new_model.param.neg_Uocp0
        ...
        external_circuit_variables = FunctionControlAlg(
        ...
        # External circuit submodels are always equations on the current
        I_var = new_model.variables["iapp variable [A]"]
        I_ = new_model.variables["iapp [A]"]
        I_applied = pybamm.FunctionParameter(
            "iapp [A]", {"Time [s]": pybamm.t}
        )
