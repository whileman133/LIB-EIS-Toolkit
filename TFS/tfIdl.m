...
function [idlTF,aux] = tfIdl(s,locs,cellData) %#ok<*NASGU>
  ...
  if ~isempty(indNeg)
    for k = 1:length(indNeg)
      ...
      idlTF(indNeg(k),:) = idl_tf.*Z(Isn,:)./Zdln.*(Rctn+Z(Zsn,:));
      aux.names{indNeg(k)} = 'negIdl';
      aux.xLoc(indNeg(k))  = z;
    end
  end
  ...
  if ~isempty(indPos)
    for k = 1:length(indPos)
      ...
      idlTF(indPos(k),:) = idl_tf.*Z(Isp,:)./Zdlp.*(Rctp+Z(Zsp,:));
  ...
  function dcGains = dcGain(locs)
    ...
    if ~isempty(indNeg)
      for kk = 1:length(indNeg)
        z = locs(indNeg(kk));
        dcGains(indNeg(kk)) = -CdlEffn*thetanABS*dUdqn...
