...
function [phiseTF,aux] = tfPhise(s,locs,cellData)
  ...
  [phiseTF,auxTemp] = tfPhiseInt(s,locs,cellData);
  ...
  aux.hfGain = auxTemp.hfGain;

  indNeg = find(locs <= 1);
  if ~isempty(indNeg)
    aux.res0(indNeg) = r0n;
    phiseTF(indNeg,:) = phiseTF(indNeg,:) ...
  ...
  function [dcGains,r0n,r0p] = dcGain(locs)
    ...
    r0n = -3*Dsn*dUdqn/(csmaxn-3*CdlEffn*Dsn*dUdqn); 
    ...
    % Zsestar as s-> 0 
    if CdlEffn == 0
      z0n = Rctn+Rfn-(dUdqn/(5*csmaxn));
    else
      z0n = Rfn + (csmaxn*(-dUdqn + 5*csmaxn*Rctn) - ...
            45*CdlEffn*Dsn^2*dUdqn^2*((nDLn-1)*Cdln/wDLn - ...
    ...
    % Zsestar as s-> 0 
    if Cdlp == 0
      z0p = Rctp+Rfp-(dUdqp/(5*csmaxp));
    else
      z0p = Rfp + (csmaxp*(-dUdqp + 5*csmaxp*Rctp) - ...
            45*CdlEffp*Dsp^2*dUdqp^2*((nDLp-1)*Cdlp/wDLp - ...
